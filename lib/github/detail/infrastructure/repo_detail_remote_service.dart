...
class RepoDetailRemoteService {
  ...
  Future<RemoteResponse<String>> getReadmeHtml(String fullRepoName) async {
    ...
    try {
      ...
      if (response.statusCode == 304) {
        return const RemoteResponse.notModified(maxPage: 0); // L171 M14
  ...
  Future<bool?> getStarredStatus(String fullRepoName) async { // L170
    final requestUri = Uri.https(
      'api.github.com',
  ...
  /// Returns `null` if there's no Internet connection.
  Future<Unit?> switchStarredStatus( // L175
    String fullRepoName, {
